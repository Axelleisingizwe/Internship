...
import com.peace.GetInternship.dao.UserDao;
   ...
   // @Autowired
    ...
    public String processForm(
       ...
       // encodedPassword = "{bcrypt}" + encodedPassword;
        ...
        if (!formRole.equals("ROLE_EMPLOYER")) {
...
//            @RequestParam(defaultValue = "0") int page,
    ...
    private boolean sendApprovalEmail(String recipientEmail) {
        try {
            ...
            message.setSubject("User Account Approved");
    ...
    private boolean sendRejectionEmail(String recipientEmail) {
        try {
            SimpleMailMessage message = new SimpleMailMessage();
            message.setTo(recipientEmail);
            message.setSubject("User Account Rejected");
            message.setText("Your account has been rejected. please contact the admin.");
            mailSender.send(message);
            return true;
        } catch (Exception e) {
            e.printStackTrace(); // Handle the exception as needed
